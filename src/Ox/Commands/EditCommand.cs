...
public static class EditCommand
{
    ...
    {
        var fileArg = new Argument<string[]>("file") { Description = "Path(s) or glob pattern(s) for .docx files (.docx only)", Arity = ArgumentArity.OneOrMore };
        ...
        var authorOption = new Option<string>("--author") { Description = "Author for tracked changes", DefaultValueFactory = _ => "ox" };
        ...
        var command = new Command("edit") { Description = "Replace, insert, or delete text in documents. Use --new \"\" to delete. Use \\n in --new to insert paragraphs. Use --track for reviewable changes (shows as red/blue in Word). Supports globs (e.g. '*.docx', '**/*.docx')" };
        ...
        command.SetActionWithErrorHandling((parseResult, ct) =>
        {
            ...
            var files = GlobExpander.Expand(patterns, GlobExpander.DocxOnly);
